...
from matplotlib.colors import ListedColormap, BoundaryNorm , to_rgba, Normalize
...
for mouse_id,metric_index in zip(mice_id,indexes_vec):
    ...
    for date, sess_name in zip(dates_vec, sessions_vec):
        ...
        _, _, pmaps_all_sessions[f'{mouse_id}'][f'{session_id}'] = paint_roi(functional_rois_dict,
...
im, _ = wf_imshow(ax_left0, pmap_avg_NF, mask=None, map=None, conv_ker=None, show_cb=False, cm_name='seismic', cb_side='left') #cm_name='seismic'
cbar = plt.colorbar(im, ax=ax_left0)
cbar.set_label(f'Z-score of delta corr {sessions_vec[1]} - corr {sessions_vec[0]}', fontsize=18)  # Set font size of colorbar label
...
ax_left0.scatter(max_index_NF[1], max_index_NF[0], marker='.', s=600, c='green') #it's plotted max_indices[1] and then
...
im, _ = wf_imshow(ax_right0, pmap_avg_control, mask=None, map=None, conv_ker=None, show_cb=False, cm_name='seismic', cb_side='left')#cm_name='seismic'
...
ax_right0.scatter(max_index_control[1], max_index_control[0], marker='.', s=600, c='green') #it's plotted max_indices[1] and then
                                                                       # [0] because [1] is the column (x) and [0] is the row (y)
...
special_cmap = plt.cm.viridis
gray_color = to_rgba("gray")  # Uniform gray color for values above the threshold
...
gray_cmap = plt.cm.gray  # Base grayscale colormap
#new_colors = gray_cmap(np.linspace(0, 1, 256))  # Extract colors from the base colormap
new_colors = np.zeros((256, 4))  # Initialize an array for RGBA colors
...
# Modify the first part (values below threshold) to use the special colormap
num_special_colors = int(threshold * 256)  # Number of colors in the special range
new_colors[:num_special_colors] = special_cmap(np.linspace(0, 1, num_special_colors))  # Scale viridis in this range
new_colors[num_special_colors:] = gray_color



# Create the new colormap
custom_cmap = ListedColormap(new_colors)
# Normalize the colorbar to only show 0 - 0.05
...
im, _ = wf_imshow(ax_right01, corrected_p_values, mask=None, map=None, conv_ker=None, show_cb=False, cm_name='viridis', cb_side='left')
cbar = plt.colorbar(im, ax=ax_right01)
#cbar.set_ticks([0,0.05])
# cbar = f.colorbar(plt.cm.ScalarMappable(norm=norm, cmap=custom_cmap), ax=ax_right01)
cbar.set_label(f'corrected pv of t-test comparing NF and Control', fontsize=18)  # Label for the colorbar
cbar.ax.tick_params(labelsize=20)
#im.set_clim(np.nanmin(pmap_avg),np.nanmax(pmap_avg))
im.set_clim(0,0.05)
#plt.colorbar(label=f'Corr {sessions_vec[1]} - corr {sessions_vec[0]}')
...
# plt.savefig(f'{base_path}/Figures_exp2.1/corr_NFandControl_with_pvmap_corrected_multiplecomparison_GREY'
#             f'{sessions_vec[1]}-'
#             f'-{sessions_vec[0]}'
