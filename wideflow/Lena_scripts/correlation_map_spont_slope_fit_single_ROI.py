...
#metric_outline = np.unravel_index(functional_rois_dict[f'roi_02']['outline'], (functional_cortex_map.shape[1], functional_cortex_map.shape[0]))
...
for i, (key, val) in enumerate(functional_rois_dict.items()):
    ...
    #metric_corr[key] = np.corrcoef(traces[key], traces[f'roi_02'])[0, 1]
...
#rois_proximity_metric = calc_rois_proximity(functional_rois_dict, f'roi_02')
...
im, _ = wf_imshow(ax_left0, pmap0, mask=None, map=None, conv_ker=None, show_cb=False, cm_name='inferno', vmin=-1, vmax=1, cb_side='left')
cbar = plt.colorbar(im, ax=ax_left0)#, ticks=[-1, 0, 1]
im.set_clim(0,1)
