...
ax.plot(metric_result, color = 'grey', alpha = 0.9, linewidth = 0.8)
#cue1 = np.array(cue)*6
plot_reward_response(ax, cue, serial_readout_correct, ymin=np.min(metric_result), ymax=np.max(metric_result))
...
plt.savefig(f'{base_path}/Figures_exp3_1/sessions with metric/{session_id}.png', format = 'png')
#plt.show()
