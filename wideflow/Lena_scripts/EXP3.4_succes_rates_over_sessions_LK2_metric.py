...
mice_and_roi = 'EXP3.5_mice_256_263_248_removed_target_ROI_norm_to_NF1_LK2_metric'
...
success_rates=success_rates.reshape(len(mice_id), 5, 3).mean(axis=2)
...
sum = []
for a,b,c,d,e in zip (norm_success_rates[4], norm_success_rates[5],norm_success_rates[6],norm_success_rates[7],
                        norm_success_rates[8]):#,norm_success_rates[9]):#,norm_success_rates[10],norm_success_rates[11]):
    sum.append(a+b+c+d+e)

mean = [num /5 for num in sum]
...
sessions_vec1 = ['NF1','NF2','NF3','NF4','NF5']#,'Control']
##PLOT LINE PER MOUSE
