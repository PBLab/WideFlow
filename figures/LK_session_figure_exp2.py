...
# load data
data = {}
with h5py.File('/data/Lena/WideFlow_prj/Results/results_exp2_noMH.h5', 'r') as f:
...
rois_metric_traces_dict = data['post_session_analysis_LK2']['zsores_MH_diff5']
...
f = plt.figure(figsize=(10,5)) #LK figsize is inches
...
#ax_bottom.legend(['metric', 'rewards timing', 'licking timing']) #, 'metric threshold'
plt.yticks(fontsize=18)
plt.xticks(fontsize=18)
axt = ax_bottom.twiny()
axt.set_xlim(ax_bottom.get_xlim())
t = np.arange(0, dt * len(metric_result), dt)
common_length = min(len(t), len(metric_result)) # added by Lena because in one mouse t was one value longer than metric
t = t[:common_length] #added by Lena because in one mouse t was one value longer than metric
metric_result = metric_result[:common_length] #added by Lena because in one mouse t was one value longer than metric
...
axt.set_xticklabels(['1', '2', '3', '4', '5'], fontsize=20)
axtx = ax_bottom.twinx()
axtx.plot(t, threshold, 'r')
axtx.tick_params(axis='y', colors='r')
ax_bottom.set_ylabel('Z-score', fontsize=20)
ax_bottom.set_xlabel('Time [minutes]', fontsize=20)
...
ax_top11.scatter(metric_outline[0], metric_outline[1], marker='.', s=10.0, c='k')
...
ax_top12.scatter(metric_outline[0], metric_outline[1], marker='.', s=10.0, c='k')
...
ax_top13.scatter(metric_outline[0], metric_outline[1], marker='.', s=10.0, c='k')
...
ax_top14.scatter(metric_outline[0], metric_outline[1], marker='.', s=10.0, c='k')
