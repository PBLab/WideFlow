# same as system_performance.py except data is taken
# from spontaneous sessions recorded with a higher frame rate
...
# using data from wide_flow_image_analysis since it has higher acquisition rate
xm = loadmat('/data/Rotem/traces.mat')
xm = np.transpose(xm['xm'])
xm_dict = {}
for i in range(1, 57):
    xm_dict[f'roi_{i:02}'] = xm[i - 1]

xm1 = loadmat('/data/Rotem/traces1.mat')
xm1 = np.transpose(xm1['xm1'])
xm1_dict = {}
for i in range(1, 57):
    xm1_dict[f'roi_{i:02}'] = xm1[i - 1]

xm2 = loadmat('/data/Rotem/traces2.mat')
xm2 = np.transpose(xm2['xm2'])
xm2_dict = {}
for i in range(1, 57):
    xm2_dict[f'roi_{i:02}'] = xm2[i - 1]
...
widths_accum = []
accum_inds_diff = []
for rois_traces in [xm_dict, xm1_dict, xm2_dict]:
    dt = 40  # factor by two for the two channels and convert to milli
...
mouse_id = '2604'
sess_id = '20220324_neurofeedback'
dataset_path = '/data/Rotem/WideFlow prj/results/sessions_20220320.h5'
data = {}
