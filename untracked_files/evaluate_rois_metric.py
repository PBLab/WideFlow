...
def smoo_rewards(inds_list, shape):
    x = np.zeros([shape[0], shape[1]-5000 + 1])
    k = np.ones((5000,))
...
dataset_path = base_path + 'results/sessions_xxx.h5'
...
for session_name in sessions_list:
    ...
    timestamp, cue, metric_result, threshold, serial_readout = extract_from_metadata_file(
        f'{base_path}{mouse_id}/{session_name}/metadata.txt')
    metadata = {"timestamp": timestamp, "cue": cue, "metric_result": metric_result, "threshold": threshold,
                "serial_readout": serial_readout}
    config = load_config(f'{base_path}{mouse_id}/{session_name}')
    sessions_metadata[session_name], sessions_config[session_name] = metadata, config
